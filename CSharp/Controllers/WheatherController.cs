...
public class WeatherController : Controller
{
    ...
    [HttpPost]
    public async Task<object> Index(AddressInput input) {
        var location = await GetLocationAsync(input);
        var weather = await GetPointWeatherAsync(location);
        return weather;
    }

    private async Task<object> GetPointWeatherAsync(LatLng latLng)
    {
        ...
        var weather = JsonConvert.DeserializeObject<WeatherPoints>(json);
